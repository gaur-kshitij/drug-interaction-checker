...
function interactionsBetweenClasses() {
  ...
  function uniquePairs() {
    $(document).ready(function() {
      $("#alprazolam").bind("click", function(event){
        alert('Success!');
        event.preventDefault();
      });
    });
  ...
  function generateListOfMedications() {
    $(document).ready(function() {
      ...
      $("#submit").bind("click", function(event) {
        ...
        //Processing
        //Push all checked medication names into array medicationsArray...
        $("input:checkbox:checked").each(function(){
          medicationsArray.push($(this).val());
        });
        //loop over each medication in medicationsArray...
        var count = 0;
        while(count < medicationsArray.length) {
          //Assign name of drug to each id correlating to what numbered loop is in process
